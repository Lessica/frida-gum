    ...
    if not frida_compile.exists():
        ...
        try:
            ...
            subprocess.run([npm, "install"] + [f"{name}@{version_spec}" for name, version_spec in RELAXED_DEPS.items()],
                           ...
                           cwd=output_dir,
                           check=True)
            subprocess.run([npm, "install", "-E"] + [
                f"{name}@{version_spec}" for name, version_spec in EXACT_DEPS.items() if not version_spec.startswith("git+")],
                           capture_output=True,
                           cwd=output_dir,
                           check=True)
            subprocess.run([npm, "install", "-E"] + [
                f"{version_spec}" for version_spec in EXACT_DEPS.values() if version_spec.startswith("git+")],
                           capture_output=True,
                           cwd=output_dir,
                           check=True)
